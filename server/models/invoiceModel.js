...
module.exports = {
    ...
    insertInvoice: (data, result) => {
        db.query("call insertInvoice2(?,?,?,?,?,?,?,?,?)", [data.firstName, data.lastName, data.phoneNumber, data.productName, data.productQuantity, data.serviceName, data.total, data.employeeID, data.payment], (err, results) => {
            if (err) {
                console.log(err);
                result(err, null);
            } else {
