    ...
    <b-container class="back">
      ...
      <table class="history" style="width:90%">
        ...
        <tr v-for="(item, index) in history" :key="index">
          ...
          <td>{{item.status}}</td>
          <td>
            <b-button variant="danger" @click="cancel(item.purchaseDate);$bvModal.show('bv-modal-example')">Cancel
            </b-button>
          </td>
        </tr>
      </table>
      <b-modal id="bv-modal-example" hide-footer>
        ...
        <b-button variant="primary" @click="$bvModal.hide('bv-modal-example'); load()" class="mt-3" block>Okay
        </b-button>
      </b-modal>
    </b-container>
  </div>
